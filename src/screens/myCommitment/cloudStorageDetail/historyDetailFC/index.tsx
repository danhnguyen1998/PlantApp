...
import {rootImageDetailFCScreen} from '../imageDetailFC/navigation';
import storage from '@react-native-firebase/storage';
import AsyncStorage from '@react-native-community/async-storage';

class HistoryDetailFCComponent extends React.Component<IProps> {
  ...
  sortByDate(arr) {
    ...
    arr.sort(function (a, b) {
      return Number(new Date(a.meta.timeCreated)) - Number(new Date(b.meta.timeCreated));
    });
  ...
  loadImage = async () => {
    ...
    storage()
      .ref(`${JSON.parse(user).user.email}/${this.props.item.name}`)
      .listAll()
      .then((result) => {
        ...
        result.items.forEach((imageRef) => {
          ...
          imageRef
            ...
            .then(async (url) => {
              ...
              console.log(this.state.images, 'ii');
            })
  ...
  goDetailImage = (item) => () => {
    rootImageDetailFCScreen(this.props.componentId, {item});
  };
            ...
            <View style={styles.wrapItemTitle}>
              ...
              <View style={{paddingRight: 10}}>
                ...
                <Text style={styles.itemTitle}>
                  {item.meta ? moment(item.meta.timeCreated).format('DD/MM/YYYY HH:mm:ss') : ''}
                </Text>
                <Text style={{fontWeight: '800', paddingHorizontal: 8}}>Location:</Text>
                <Text style={styles.itemTitle}>
                  {item.meta.customMetadata ? item.meta.customMetadata.location : ''}
                </Text>
              </View>
            </View>
          </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(HistoryDetailFCComponent);
