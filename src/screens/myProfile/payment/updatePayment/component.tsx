...
import {updateCardAction} from '../redux/actions';
import {IProps, IState} from './propState';
import styles from './styles';
import base64 from 'react-native-base64';
import {CheckBox} from 'react-native-elements';

export default function UpdatePaymentComponent(props: IProps) {
  ...
  props = useSelector<RootState, IProps>((state: RootState) => ({
    ...
    updateCardAction: (card_id: string, four_digit_card: string) =>
      dispatch(updateCardAction(card_id, four_digit_card)),
  }));
      ...
      if (validCard.isValid) {
        ...
        if (res.status === System.RESPONSE_STATUS.SUCESS) {
          ...
          if (resCard.status === System.RESPONSE_STATUS.CREATED) {
            ...
            props.updateCardAction(cardInfor.id, cardInfor.number);
            _goBack();
          } else {
